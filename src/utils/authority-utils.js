...
function filterMenuWithPermision(menuData) {
  return menuData.filter(menu => {
    if (menu.permission) {
      if (!checkUserPermission(menu.permission)) {
        return false
      }
    }
    if (menu.children && menu.children.length > 0) {
      menu.children = filterMenuWithPermision(menu.children)
    }
    return true
  })
}

export {filterMenu, hasAuthority, filterMenuWithPermision}
