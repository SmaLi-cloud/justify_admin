  ...
  watch: {
    ...
    '$route': function (newRoute) {
      ...
      if (!this.multiPage) {
        ...
        let index = 0;
        this.pageList = this.pageList.filter((item, i) => {
          if (item.keyPath === newRoute.path) {
            this.clearCache(item)
          }
          if (item.twoLevelPath && item.twoLevelPath === newRoute.matched[2]?.path) {
            index = i;
          }
          return item.keyPath !== newRoute.path
        });
        if (index > 0 && !newRoute.meta.openNew) {
          this.pageList.splice(index, 1, this.createPage(newRoute));
        } else {
  ...
  methods: {
    ...
    createPage(route) {
      return {
        ...
        twoLevelPath: route.matched[2]?.path,
        fullPath: route.fullPath, loading: false,
        title: (route.meta && route.meta.page && route.meta.page.title) || route.query.customname,
