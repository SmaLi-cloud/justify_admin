...
export default {
  ...
  computed: {},
  methods: {
    ...mapActions("technology", ["technologyDetail"]),
    getDetailValue() {
      this.technologyDetail({
        ...
        let baseInfo = {
          ...
          样品数量: productInfo.sampleQuantity,
          样品金额: productInfo.sampleAmount,
        ...
        let specifList = [];
        specif.map((item) => {
          const { sampleLocationInfo } = item;
          if (sampleLocationInfo && sampleLocationInfo.length) {
            sampleLocationInfo.map((locationItem, index) => {
              let obj = {
                ...item,
                ...locationItem,
              };
              if (sampleLocationInfo.length > 1) {
                if (index === 0) {
                  obj.rowSpan = sampleLocationInfo.length;
                } else {
                  obj.rowSpan = 0;
                }
              } else {
